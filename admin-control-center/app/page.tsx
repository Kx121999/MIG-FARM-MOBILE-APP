...
async function openOrder(session: Session, id: string, setDetail: (value: { title: string; content: React.ReactNode }) => void, setToast: (toast: Toast) => void) {
  try {
    ...
    setDetail({ title: data.order.id, content: <div><p>{data.order.customer?.name || 'Guest'} · Payment: {data.order.paymentStatus} · Fulfillment: {data.order.fulfillmentStatus}</p><div className="bar health"><span>Odoo</span><Badge value={data.order.odooSyncStatus === 'synced' ? 'Connected' : data.order.odooSyncStatus || 'Pending'} /></div><p>Quotation: {data.order.odooOrderName || 'Not connected'}<br />Odoo state: {data.order.odooState || 'Pending'}<br />Stripe PaymentIntent: {data.order.paymentIntentId || 'Not created'}<br />Sync issue: {data.order.odooSyncError || 'None'}<br />Last sync: {data.order.odooSyncedAt ? prettyDate(data.order.odooSyncedAt) : 'Never'}</p>{data.order.odooSyncStatus !== 'synced' ? <button className="btn mini" onClick={retryOdoo}>Retry Odoo Sync</button> : null}<Table columns={['Item', 'Variant', 'Qty', 'Unit', 'Line']} empty="No items" rows={(data.order.items || []).map((item: any, index: number) => ({ id: String(index), cells: [item.title, item.variant_title || '-', item.quantity, item.unit_price, item.line_total] }))} /><p>Subtotal: AED {data.order.subtotal}<br />Tax: AED {data.order.tax || 0}<br />Delivery: AED {data.order.delivery}<br />Total: AED {data.order.total}</p></div> });
  } catch (error) {
