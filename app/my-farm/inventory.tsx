...
export default function FarmInventoryScreen() {
  ...
  const save = async () => {
    ...
    try {
      await farmService.createInventoryItem({ farmId, productNameSnapshot: name.trim(), category: category.trim(), quantity: numericQuantity, unit: unit.trim() || null, notes: notes.trim() });
      setName(''); setCategory(''); setQuantity(''); setUnit(''); setNotes(''); setAdding(false);
      await load();
